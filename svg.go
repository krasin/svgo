...
<svg width="%d" height="%d"`
	svginitMm  = `<?xml version="1.0"?>
<!-- Generated by SVGo -->
<svg width="%dmm" height="%dmm"`
	svgns      = `
...
func (svg *SVG) StartMm(w int, h int, ns ...string) {
	svg.printf(svginitMm, w, h)
	for _, v := range ns {
		svg.printf("\n     %s", v)
	}
	svg.println(svgns)
...
// Startview begins the SVG document, with the specified width, height, and viewbox 
func (svg *SVG) StartviewMm(w, h, minx, miny, vw, vh int) {
	svg.StartMm(w, h, fmt.Sprintf(`viewBox="%d %d %d %d"`, minx, miny, vw, vh))
}
